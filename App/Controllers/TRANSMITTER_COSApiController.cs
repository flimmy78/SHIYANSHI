...
namespace Langben.App.Controllers
{
    ...
    /// 变送器cos
    /// </summary>
    public class TRANSMITTER_COSApiController : BaseApiController
    {
        /// <summary>
        /// 异步加载数据
        /// </summary>
        ...
        /// <returns></returns>
        public TRANSMITTER_COS Get(string id)
        {
            TRANSMITTER_COS item = m_BLL.GetById(id);
            return item;
        }
        ...
        /// <returns></returns>
        public Common.ClientResult.Result Post([FromBody]TRANSMITTER_COS entity)
        {           
            ...
            if (entity != null && ModelState.IsValid)
            {
               ...
               entity.CREATETIME = DateTime.Now;
                ...
                {
                    LogClassModels.WriteServiceLog(Suggestion.InsertSucceed  + "，变送器cos的信息的Id为" + entity.ID,"变送器cos"
                        );//写入日志 
                ...
                { 
                    ...
                    LogClassModels.WriteServiceLog(Suggestion.InsertFail + "，变送器cos的信息，" + returnValue,"变送器cos"
                        );//写入日志                      
        ...
        /// <returns></returns>  
        public Common.ClientResult.Result Put([FromBody]TRANSMITTER_COS entity)
        {
            Common.ClientResult.Result result = new Common.ClientResult.Result();
            if (entity != null && ModelState.IsValid)
            {   //数据校验
                ...
                {
                    LogClassModels.WriteServiceLog(Suggestion.UpdateSucceed + "，变送器cos信息的Id为" + entity.ID,"变送器cos"
                        );//写入日志                   
                    result.Code = Common.ClientCode.Succeed;
                ...
                else
                {
                    ...
                    LogClassModels.WriteServiceLog(Suggestion.UpdateFail + "，变送器cos信息的Id为" + entity.ID + "," + returnValue, "变送器cos"
                        );//写入日志   
                    result.Code = Common.ClientCode.Fail;
        ...
        IBLL.ITRANSMITTER_COSBLL m_BLL;

        ValidationErrors validationErrors = new ValidationErrors();

        public TRANSMITTER_COSApiController()
            : this(new TRANSMITTER_COSBLL()) { }

        public TRANSMITTER_COSApiController(TRANSMITTER_COSBLL bll)
        {
            m_BLL = bll;
        }
