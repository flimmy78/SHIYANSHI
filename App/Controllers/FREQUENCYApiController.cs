...
namespace Langben.App.Controllers
{
    ...
    /// 频率表
    /// </summary>
    public class FREQUENCYApiController : BaseApiController
    {
        /// <summary>
        /// 异步加载数据
        /// </summary>
        ...
        /// <returns></returns>
        public FREQUENCY Get(string id)
        {
            FREQUENCY item = m_BLL.GetById(id);
            return item;
        }
        ...
        /// <returns></returns>
        public Common.ClientResult.Result Post([FromBody]FREQUENCY entity)
        {           
            ...
            if (entity != null && ModelState.IsValid)
            {
                ...
                {
                    LogClassModels.WriteServiceLog(Suggestion.InsertSucceed  + "，频率表的信息的Id为" + entity.ID,"频率表"
                        );//写入日志 
                ...
                { 
                    ...
                    LogClassModels.WriteServiceLog(Suggestion.InsertFail + "，频率表的信息，" + returnValue,"频率表"
                        );//写入日志                      
        ...
        /// <returns></returns>  
        public Common.ClientResult.Result Put([FromBody]FREQUENCY entity)
        {
            Common.ClientResult.Result result = new Common.ClientResult.Result();
            if (entity != null && ModelState.IsValid)
            {   //数据校验
                ...
                {
                    LogClassModels.WriteServiceLog(Suggestion.UpdateSucceed + "，频率表信息的Id为" + entity.ID,"频率表"
                        );//写入日志                   
                    result.Code = Common.ClientCode.Succeed;
                ...
                else
                {
                    ...
                    LogClassModels.WriteServiceLog(Suggestion.UpdateFail + "，频率表信息的Id为" + entity.ID + "," + returnValue, "频率表"
                        );//写入日志   
                    result.Code = Common.ClientCode.Fail;
        ...
        IBLL.IFREQUENCYBLL m_BLL;

        ValidationErrors validationErrors = new ValidationErrors();

        public FREQUENCYApiController()
            : this(new FREQUENCYBLL()) { }

        public FREQUENCYApiController(FREQUENCYBLL bll)
        {
            m_BLL = bll;
        }
