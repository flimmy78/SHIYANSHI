...
{
    /// <summary>
    /// ^ReplaceClassCOMPANYNAME^
    /// </summary>
    public class COMPANYTreeNodeCollection
    {
        IEnumerable<COMPANY> listTree;
        public bool Bind(IEnumerable<COMPANY> entitys, string myPARENTID, ref List<SystemTree> myChildren)
        {
            if (null != myPARENTID)
            ...
            {//填充数据
                ...
                {
                    SystemTree myTree = new SystemTree() { id = item.ID.GetString(), text = item.COMPANYNAME.GetString() };
