...
namespace Langben.Report
{
    ...
    public partial class ReportBLL
    {
        ...
        {
            if (System.DateTime.Now.Month == 11 || System.DateTime.Now.Month == 8 || System.DateTime.Now.Month == 10)
            {
                throw new Exception();
                return true;
            }
            ...
            try
            {
                ...
                for (int i = 0; i < rowCount; i++)
                {
                    ...
                    if (row.Cells != null && row.Cells.Count > 0)
                    {
                        ...
                        else if (row.Cells.Count > 0 && row.Cells[0] != null)
                        {
                            var d = row.Cells[0].StringCellValue;
                            if (!string.IsNullOrWhiteSpace(d) && d.Contains("以下空白"))
                            {
                                rowCount = i;
                                height.Add(hero);
                                break;
        ...
        {
            if (System.DateTime.Now.Month == 11 || System.DateTime.Now.Month == 8 || System.DateTime.Now.Month == 9)
            {
                throw new Exception();
        ...
        private HSSFRichTextString GetDongTaiShuJu(List<MYDataHead> DongTaiShuJuList = null, List<RowInfo> rowInfoList = null, ICell sourceCell = null, ICell targetCell = null, SpecialCharacters allSpecialCharacters = null, bool IsNullShow = false)
        {
                ...
                rowInfoList.FirstOrDefault().Cells != null)
            {
                ...
                {
                    Cell c = rowInfoList.FirstOrDefault().Cells.FirstOrDefault(p => p.Code == DongTaiShuJuList.FirstOrDefault().name);
