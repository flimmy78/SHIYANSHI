...
namespace Langben.DAL
{
    ...
    /// 直流电压输出
    /// </summary>
    public partial class DC_VOLTAGE_OUTPUTRepository : BaseRepository<DC_VOLTAGE_OUTPUT>, IDisposable
    {
        /// <summary>
        /// 查询的数据
        /// </summary>
        ...
        /// <returns></returns>      
        public IQueryable<DC_VOLTAGE_OUTPUT> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<DC_VOLTAGE_OUTPUT>().Where(string.IsNullOrEmpty(where) ? "true" : where)
        ...
        /// <param name="id">一条直流电压输出的主键</param>
        public void Delete(SysEntities db, string id)
        {
            DC_VOLTAGE_OUTPUT deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.DC_VOLTAGE_OUTPUT.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<DC_VOLTAGE_OUTPUT> collection = from f in db.DC_VOLTAGE_OUTPUT
