...
namespace Langben.DAL
{
    ...
    /// 误差限
    /// </summary>
    public partial class ERROR_LIMITRepository : BaseRepository<ERROR_LIMIT>, IDisposable
    {
        /// <summary>
        /// 查询的数据
        /// </summary>
        ...
        /// <returns></returns>      
        public IQueryable<ERROR_LIMIT> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<ERROR_LIMIT>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取误差限---查看详细，首次编辑
        /// </summary>
        ...
        /// <param name="id">主键</param>
        /// <returns>误差限</returns>
        public ERROR_LIMIT GetById(SysEntities db, string id)
        { 
            return db.ERROR_LIMIT.SingleOrDefault(s => s.ID == id);
        ...
        /// <param name="id">一条误差限的主键</param>
        public void Delete(SysEntities db, string id)
        {
            ERROR_LIMIT deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.ERROR_LIMIT.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<ERROR_LIMIT> collection = from f in db.ERROR_LIMIT
