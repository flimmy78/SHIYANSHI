...
namespace Langben.DAL
{
    ...
    /// 直流电流输出
    /// </summary>
    public partial class DC_CURRENT_OUTPUTRepository : BaseRepository<DC_CURRENT_OUTPUT>, IDisposable
    {
        /// <summary>
        /// 查询的数据
        /// </summary>
        ...
        /// <returns></returns>      
        public IQueryable<DC_CURRENT_OUTPUT> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<DC_CURRENT_OUTPUT>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取直流电流输出---查看详细，首次编辑
        /// </summary>
        /// <param name="id">主键</param>
        /// <returns>直流电流输出</returns>
        ...
        /// <param name="id">主键</param>
        /// <returns>直流电流输出</returns>
        public DC_CURRENT_OUTPUT GetById(SysEntities db, string id)
        { 
            return db.DC_CURRENT_OUTPUT.SingleOrDefault(s => s.ID == id);
        ...
        /// <param name="id">一条直流电流输出的主键</param>
        public void Delete(SysEntities db, string id)
        {
            DC_CURRENT_OUTPUT deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.DC_CURRENT_OUTPUT.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<DC_CURRENT_OUTPUT> collection = from f in db.DC_CURRENT_OUTPUT
        ...
        /// <summary>
        /// 根据OVERALL_TABLEID，获取所有直流电流输出数据
        /// </summary>
        /// <param name="id">外键的主键</param>
        /// <returns></returns>
        public IQueryable<DC_CURRENT_OUTPUT> GetByRefOVERALL_TABLEID(SysEntities db, string id)
        {
            return from c in db.DC_CURRENT_OUTPUT
