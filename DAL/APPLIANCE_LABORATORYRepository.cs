...
namespace Langben.DAL
{
    ...
    public partial class APPLIANCE_LABORATORYRepository : BaseRepository<APPLIANCE_LABORATORY>, IDisposable
    {
        ...
        /// <returns></returns>      
        public IQueryable<APPLIANCE_LABORATORY> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            if (queryDic != null && queryDic.Count > 0)
            {
                foreach (var item in queryDic)
                {
                    ...
                    if (queryDic.ContainsKey("APPLIANCE_DETAIL_INFORMATIOID") && !string.IsNullOrWhiteSpace(item.Key) && !string.IsNullOrWhiteSpace(item.Value) && item.Value == "noway" && item.Key == "APPLIANCE_DETAIL_INFORMATIOID")
                    {//查询一对多关系的列名
                        where += "it.APPLIANCE_DETAIL_INFORMATIOID is null";
                        continue;
                    }
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<APPLIANCE_LABORATORY>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取器具明细信息_承接实验室---查看详细，首次编辑
        /// </summary>
        /// <param name="id">主键</param>
        /// <returns>器具明细信息_承接实验室</returns>
        ...
        /// <param name="id">主键</param>
        /// <returns>器具明细信息_承接实验室</returns>
        public APPLIANCE_LABORATORY GetById(SysEntities db, string id)
        { 
            return db.APPLIANCE_LABORATORY.SingleOrDefault(s => s.ID == id);
        ...
        /// <param name="id">一条器具明细信息_承接实验室的主键</param>
        public void Delete(SysEntities db, string id)
        {
            APPLIANCE_LABORATORY deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.APPLIANCE_LABORATORY.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        /// <summary>
        /// 根据UNDERTAKE_LABORATORYID，获取所有器具明细信息_承接实验室数据
        /// </summary>
        /// <param name="id">外键的主键</param>
        /// <returns></returns>
        public IQueryable<APPLIANCE_LABORATORY> GetByRefUNDERTAKE_LABORATORYID(SysEntities db, string id)
        {
        ...
        /// <summary>
        /// 根据APPLIANCE_DETAIL_INFORMATIOID，获取所有器具明细信息_承接实验室数据
        /// </summary>
        /// <param name="id">外键的主键</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 根据PREPARE_SCHEMEID，获取所有器具明细信息_承接实验室数据
        /// </summary>
        /// <param name="id">外键的主键</param>
        /// <returns></returns>
        public IQueryable<APPLIANCE_LABORATORY> GetByRefPREPARE_SCHEMEID(SysEntities db, string id)
        {
            return from c in db.APPLIANCE_LABORATORY
